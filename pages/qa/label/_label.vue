  ...
  <div class="wrapper qa-content"> 
   ...
   <div class="fl right-tag"> 
    ...
    <div class="hot-tags"> 
     ...
     <div class="tags"> 
      <ul class="sui-tag" v-for="(label,index) in hotlabel" :key="index"> 
...
export default {
  asyncData({params}){
    return axios.all([problemApi.listByLabelId('newlist',params.label,1,10),
      problemApi.listByLabelId('hotlist',params.label,1,10),
        problemApi.listByLabelId('waitlist',params.label,1,10) ,
        labelApi.toplist() ] ).then( axios.spread(function(newlist,hotlist,waitlist,hotlabel ){
          return {
            newlist:newlist.data.data.rows,
            hotlist:hotlist.data.data.rows,
            waitlist:waitlist.data.data.rows,
            hotlabel:hotlabel.data.data,
            label:params.label
          }
        }))
  },
  ...
  methods:{
    loadMore(){
      if(this.type==='new'){
        this.page_new++
        problemApi.listByLabelId('newlist',this.label,this.page_new,10).then( res=>{
          this.newlist=this.newlist.concat( res.data.data.rows )
        })
      }
      if(this.type==='hot'){
        this.page_hot++
        problemApi.listByLabelId('hotlist',this.label,this.page_hot,10).then( res=>{
          this.hotlist=this.hotlist.concat( res.data.data.rows )
        })
      }
      if(this.type==='wait'){
        this.page_wait++
        problemApi.listByLabelId('waitlist',this.label,this.page_wait,10).then( res=>{
