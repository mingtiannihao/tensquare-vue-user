      ...
      <div class="detail-tit"> 
       ...
       <div class="detail-tool"> 
        <ul> 
         ...
         <li><a data-toggle="modal" data-target="#remarkModal"  @click="dialogVisible=true;content=''"><i  class="fa fa-commenting" aria-hidden="true"></i> {{pojo.comment}}</a></li> 
        </ul> 
       </div> 
      </div> 
...
export default {
    asyncData (params) {
       return axios.all([spitApi.findById(params.id), spitApi.commentlist(params.id)]).then(
          axios.spread(function(pojo,commentlist){
              return {pojo:pojo.data.data,commentlist:commentlist.data.data.rows}
          })
       )
    }, 
    data(){
    ...
    save() {
        spitApi.submit({ content: this.content, parentid: this.pojo.id }).then(res => {
            this.$message({
